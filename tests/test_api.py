...
from app.api.routes import format_github_response, handle_github_query
...
@pytest.mark.asyncio
async def test_github_summary_handles_hyphenated_repo_names():
    """Summary queries should preserve full owner/repo names that contain hyphens."""
    summary_content = {
        "repo": "shrutiusonavane/cafe-finder",
        "files": {
            "README.md": "# Cafe Finder\nA web app for finding nearby cafes."
        },
        "error": None,
    }

    with patch(
        "app.api.routes.github_mcp_service.get_repo_summary_content",
        new=AsyncMock(return_value=summary_content),
    ) as mock_summary, patch(
        "app.api.routes.ollama_client.ask",
        new=AsyncMock(return_value="Cafe Finder helps users find nearby cafes."),
    ) as mock_ask:
        response = await handle_github_query("summarize shrutiusonavane/cafe-finder")

    mock_summary.assert_awaited_once_with(
        owner="shrutiusonavane",
        repo="cafe-finder",
        branch="main",
    )
    assert mock_ask.await_count == 1
    assert response == "Cafe Finder helps users find nearby cafes.\n\nRepository scope: based on README.md."


def test_format_github_response_formats_empty_search_json_text():
    """Raw GitHub search JSON should not be shown directly in chat."""
    response = format_github_response({
        "result": {
            "content": [
                {
                    "type": "text",
                    "text": '{"total_count": 0, "incomplete_results": false, "items": []}',
                }
            ]
        }
    })

    assert response == "I could not find any matching GitHub repositories."
